    ...
    <form action="{{ route('register.post') }}" method="post" class="grid gap-2 w-96" novalidate>
        ...
        <div>
            ...
            @error('password')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
            <input type="password" class="block w-full rounded p-2 bg-gray-100" value="{{ old('password') }}"
                name="password">
        </div>
        <div>
            ...
            @error('password_confirmation')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
            <input type="password" class="block w-full rounded p-2 bg-gray-100" value="{{ old('password_confirmation') }}"
